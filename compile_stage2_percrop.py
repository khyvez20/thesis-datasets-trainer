...
-------------------------
Unlike the Stage 1 detector, the classifier HEF is compiled WITHOUT a baked
/255 normalization layer. The runtime (hailo_runner.py, baked_norm=False) will
pre-divide the input by 255 and send float32 [0-1] to the chip. Do NOT add
a normalization .alls line here — HailoRT handles the float↔uint8 mapping.
"""
...
# NO baked normalization — classifier HEF receives float32 [0-1] at runtime.
# hailo_runner.py (baked_norm=False) divides by 255 before inference.
...
def compile_classifier(crop: str, onnx_path: str, calib_dir: str):
    ...
    print(f"  norm baked: {BAKE_NORMALIZATION}  (runtime divides by 255)")
    print("=" * 60)
    ...
    print("  model script loaded (no baked normalization).")
